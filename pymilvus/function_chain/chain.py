...
Examples:
    ...
    results::
        ...
        from pymilvus.function_chain import FunctionChain, FunctionChainExpr
        from pymilvus.function_chain import FunctionChainStage, col

        chain = (
            FunctionChain(FunctionChainStage.L0_RERANK, name="score_rerank")
            .map(
                "score",
                FunctionChainExpr(
                    "weighted_sum",
                    args=(col("dense_score"), col("sparse_score")),
...
class FunctionChainStage(IntEnum):
    ...
    Examples:
        Create a chain for the first reranking stage::

            chain = FunctionChain(FunctionChainStage.L0_RERANK)
    """
...
class ColumnRef:
    ...
    Examples:
        Use ``ColumnRef`` directly or create one with :func:`col`::
            ...
            expr = FunctionChainExpr("boost", args=(score_ref,), params={"factor": 1.2})
    """
...
def col(name: str) -> ColumnRef:
    ...
    Examples:
        Pass collection fields as expression inputs::
            ...
            expr = FunctionChainExpr("normalize", args=(col("score"),))
    """
    ...
    if isinstance(value, Mapping):
        return schema_pb2.FunctionParamValue(
            object_value=schema_pb2.FunctionParamObject(
                fields={str(key): _to_param_value(item) for key, item in value.items()}
            )
        )
...
class FunctionChain:
    ...
    def map(self, output: str, expr: FunctionChainExpr) -> "FunctionChain":
        ...
        Examples:
            Compute a normalized score into a new output field::

                chain.map("normalized_score", FunctionChainExpr("normalize", args=(col("score"),)))
        """
